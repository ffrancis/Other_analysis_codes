...
selected_line_column = get_line_column_no(selected_line, 30)
skip_list = range(0,20)
df = pd.read_csv(input_path + input_file, sep='\t', skiprows=skip_list)
df = df[(df.POS >= int(locus_start)) & (df.POS <= int(locus_stop))]

# print df

df.to_csv("ALL_GENOTYPES_subsetted_" + input_file[:-4] + ".txt", sep='\t', encoding='utf-8', index=False)
